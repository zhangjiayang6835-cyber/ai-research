# Placeholder for sidecar injection fix - see src/session_crypto.py for padding oracle fix
...
"""
Secure session cookie encryption/decryption using AES-GCM.
Fixes Padding Oracle vulnerability by using authenticated encryption
instead of AES-CBC with PKCS7 padding.
"""
...
class SecureSessionManager:
    """
    Secure session manager that uses AES-GCM for authenticated encryption.
    This prevents padding oracle attacks by:
    1. Using authenticated encryption (GCM) instead of unauthenticated CBC
    2. Eliminating padding entirely - GCM uses CTR mode internally
    3. Providing integrity protection - tampered ciphertext is rejected
    """
    
    def __init__(self, key: bytes = None):
        """Initialize with a 256-bit key. Generates random key if not provided."""
        if key is None:
    ...
    def encrypt_session(self, plaintext: str) -> str:
        """
    ...
    def decrypt_session(self, token: str) -> str:
        """
        Decrypt and verify session data.
        Raises ValueError on tampering or decryption failure.
        """
    ...
    def create_session_cookie(self, user_id: str, role: str = "user") -> str:
        """Create an encrypted session cookie."""
        session_data = f"user_id={user_id}&role={role}"
        return self.encrypt_session(session_data)
    
    def validate_session_cookie(self, cookie: str) -> dict:
        """Validate and parse a session cookie."""
        try:
            data = self.decrypt_session(cookie)
            result = {}
            ...
            # Never expose decryption failures - prevents oracle attacks
            raise ValueError("Invalid session")


# Backward-compatible API for existing code
def encrypt_cookie(plaintext: str, key: bytes = None) -> str:
    """Encrypt a cookie value securely."""
    manager = SecureSessionManager(key)
    return manager.encrypt_session(plaintext)


def decrypt_cookie(ciphertext: str, key: bytes = None) -> str:
    """Decrypt a cookie value securely."""
    manager = SecureSessionManager(key)
    return manager.decrypt_session(ciphertext)
