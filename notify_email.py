...
def sign_email_with_dkim(msg_bytes, selector, domain, private_key):
    ...
    try:
        ...
        signed_msg = dk.sign(message=msg_bytes, domain=domain, selector=selector)
        return signed_msg
    except Exception as e:
        print(f"Failed to sign email: {e}")
        return None
...
def generate_secure_state_token():
    """Generate a secure state token."""
    return ''.join(random.choices(string.ascii_uppercase + string.digits, k=16))

if __name__ == "__main__":
    state_token = generate_secure_state_token()
    print(f"Generated Secure State Token: {state_token}")